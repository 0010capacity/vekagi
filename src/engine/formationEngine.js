 ...
 * (초기 배치 위치와 비교)
 */
export function checkFormationMaintained(currentPieces, initialPositions, totalSlots) {
 ...
 */
export function getFormationEffectMultiplier(status) {
    switch (status) {
 ...
 */
export function applyFormationPassives(pieces, _formation, maintainStatus) {
    if (maintainStatus === 'broken')
        return pieces; // 붕괴 시 효과 없음
 ...
 * 진형 슬롯에서 코어 슬롯인지 확인
 */
export function isCoreSlot(formation, slotIndex) {
    return slotIndex < formation.coreSlots;
}
/**
 * 진형 패턴 파싱 (그리드 문자열 → 2차원 배열)
 */
export function parseFormationPattern(pattern) {
