...
// 런 맵 생성 (순수 함수)
const ZONE_FLOORS = { 1: 10, 2: 10, 3: 10 };
/**
...
function pickNodeType(zone, floor) {
    if (floor === 10)
        return 'boss';
    ...
    // 비율: 전투 > 정예 > 거점 ≈ 상인 ≈ 이벤트
    if (roll < 1 - eliteChance - restChance - shopChance - eventChance)
        return 'battle';
    if (roll < 1 - restChance - shopChance - eventChance)
        return 'elite';
    if (roll < 1 - shopChance - eventChance)
        return 'rest';
    if (roll < 1 - eventChance)
        return 'shop';
    return 'event';
}
...
export function moveToNode(map, nodeId) {
    ...
    return {
        ...map,
        nodes: map.nodes.map(n => n.id === map.currentNodeId
            ? { ...n, completed: true }
            : n),
