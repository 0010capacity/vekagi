 ...
 */
export function tickCountdowns(pieces) {
    const events = [];
 ...
 * 봉인(seal) 타이머 감소
 */
export function tickSealedStatus(pieces) {
    return pieces.map(piece => {
        ...
        return {
            ...piece,
            statusEffects: piece.statusEffects.map(e => e.type === 'sealed' ? { ...e, remainingTurns: newTurns } : e),
        };
    });
}
/**
 * 기물에 상태 효과 추가
 */
export function addStatusEffect(piece, effect) {
    const existing = piece.statusEffects.find(e => e.type === effect.type);
    if (existing) {
        // 기존 효과 갱신 (더 긴 지속시간 or 더 큰 값)
        return {
            ...piece,
            statusEffects: piece.statusEffects.map(e => e.type === effect.type
                ? {
                    ...e,
