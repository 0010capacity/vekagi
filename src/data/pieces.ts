...
export const getPieceById = (id: number): PieceDefinition | undefined =>
  PIECES.find(p => p.id === id);

export const getPiecesByCategory = (category: PieceDefinition['category']): PieceDefinition[] =>
  PIECES.filter(p => p.category === category);

export const MOVE_TYPES = ["주(主)", "사(斜)", "전(全)", "십자(十)", "사선(╲)", "도약(♞)", "전진(↑)", "역주(↓)", "돌진(⇒)", "고정(■)", "랜덤(?)"] as const;
