...
self.onmessage = (e) => {
    ...
    if (type === 'compute_turn') {
        ...
        const response = { type: 'turn_computed', actions, intentArrows };
        self.postMessage(response);
    }
};
// ── 행동 결정 ──────────────────────────────────────────
function computeRuleBasedTurn(gameState, aiPieces, archetype, difficulty) {
    ...
    for (const aiPiece of aiPieces.filter(p => !p.isDead && !p.isSealed)) {
        ...
        const validMoves = getValidMoves(aiPiece, gameState.board, moveType);
...
function findBestMove(aiPiece, validMoves, playerPieces, gameState, archetype, difficulty) {
...
function evaluateMove(aiPiece, targetPos, playerPieces, gameState, archetype, difficulty) {
    let score = 0;
    ...
    for (const player of playerPieces) {
        ...
        // 인접 여부 (충돌 가능)
        const isAdjacent = Math.abs(player.position.row - targetPos.row) +
        ...
        if (chainForce > 0) {
            const chainTarget = gameState.playerPieces.find(p => p.position.row === finalRow && p.position.col === finalCol && !p.isDead);
            if (chainTarget)
                score += 100 * weights.chain;
        }
        // 우선순위 4: 가장자리로 유도 (+30)
...
// ── 의도 화살표 생성 ────────────────────────────────────
function generateIntentArrows(actions, pieces) {
